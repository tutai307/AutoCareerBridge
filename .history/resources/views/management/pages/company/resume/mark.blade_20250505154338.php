...
@if(session('score') && session('review'))
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Tách văn bản thành các phần thường và in đậm
            var parts = [];
            var currentPos = 0;
            var boldRegex = /\*\*(.*?)\*\*/g;
            var match;

            while ((match = boldRegex.exec(reviewText)) !== null) {
                // Thêm văn bản thường trước phần in đậm
                if (match.index > currentPos) {
                    parts.push({
                        text: reviewText.substring(currentPos, match.index),
                        bold: false
                    });
                }
                // Thêm phần in đậm
            ...
            if (currentPos < reviewText.length) {
                parts.push({
                    text: reviewText.substring(currentPos),
                    bold: false
                });
            }

            function typeWriter() {
                if (i < parts.length && typing) {
                    var part = parts[i];
                    var text = part.text;
                    if (part.bold) {
                        reviewContainer.innerHTML += `<strong>${text}</strong>`;
                    } else {
                        reviewContainer.innerHTML += text.replace(/\n/g, '<br>');
                    }
                    i++;
