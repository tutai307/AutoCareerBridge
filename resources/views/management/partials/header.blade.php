<div class="nav-header">
    @php
        ...
        } elseif (Auth::guard('admin')->user()->role === ROLE_UNIVERSITY || Auth::guard('admin')->user()->role === ROLE_SUB_UNIVERSITY) {
            $checkRoute = 'university.home';
        }
    @endphp
    <a href="{{ route($checkRoute) }}"><img src=" {{ asset('clients/images/header/logo2.png') }}"
                                            alt="Logo" title="Job Pro" class=" brand-logo">
    </a>
                        ...
                        <div class="dropdown-menu dropdown-menu-end of-visible " data-bs-popper="static">
                            ...
                            @if (count($notificationsHeader) > 0)
                                <a class="all-notification"
                                   href="{{ route('notifications') }}">{{ __('label.notification.read_all') }}<i
