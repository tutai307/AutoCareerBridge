...
@section('content')
            ...
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title"
                        id="detailsModalLabel">{{ __('label.company.collaboration.detail_colab') }}
                    </h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body" id="detailsModalBody">
                    <!-- Form bên trong modal -->
                    <form action="{{ route('company.changeStatusColab') }}" id="jobForm" method="POST">
                        @csrf
                        @method('POST')
                        <div class="row">
                            <div class="col-md-12 col-xl-3">
                                <div class="card box-2 pt-0" style="max-height: 240px;">
                                    <div class="flow">
                                        <div class="dz-media"><img
                                            ...
                                            <p id="university-size">{{ __('label.company.collaboration.size') }}:
                                                ssss</p>
                                        </div>
                                ...
                                <div class="card box-2 pt-0" style="max-height: 240px;" id="feedback-div">
                                    <div class="flow">
                                        <div class="dz-media">
                                            <h5>{{ __('label.company.collaboration.feedback') }}</h5>
                                            <p id="res_message"></p>
                                        </div>
                            ...
                            <div class="col-md-12 col-xl-9">
                                <div class="card box-1 overflow-hidden">
                                    <div class="max-2 mt-3">
                                        <h2 class=" mb-3" id="title-colab">
                                            sdsdssd
                                        </h2>
                                        <div class="ul-li">
                                            <ul class="d-flex mb-2">
                                                <li class="me-3 me-lg-5">
                                                    <svg class="me-2" width="24"
                                        ...
                                        <h3 class="card-title mb-3">
                                            {{ __('label.company.collaboration.detail_colab') }}</h3>
                ...
                <div class="modal-footer" id="buttonSubmit">
                    <button type="button" class="btn btn-danger light" onclick="openModal()" data-status='3'
                            id="btnReject">{{ __('label.company.collaboration.reject') }}</button>
                    <button type="button" class="btn btn-primary " onclick="submitForm(2)"
                            id="btnSubmit">{{ __('label.company.collaboration.approve') }}</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="modal fade" id="rejectModal" tabindex="-1" aria-labelledby="rejectModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="rejectModalLabel">{{ __('label.company.collaboration.feedback') }}
                    </h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <form id="rejectForm" action="{{ route('company.changeStatusColab') }}" method="POST">
                        @csrf
                        <input type="hidden" id="id-res" name="id" value="a">
                        <input type="hidden" name="status" value="3">
                        <div class="mb-3">
                            <label for="feedbackContent"
                                   class="form-label">{{ __('label.company.collaboration.feedback_content') }}</label>
                            <textarea class="form-control" id="feedbackContent"
                                      placeholder="{{ __('label.company.collaboration.feedback_placeholder') }}"
                                      name="res_message"
                                      rows="3"></textarea>
                        </div>
                    </form>
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-light"
                            data-bs-dismiss="modal">{{ __('label.company.collaboration.cancel') }}</button>
                    <button type="button" class="btn btn-primary"
                            id="sendFeedback">{{ __('label.company.collaboration.send') }}</button>
                </div>
            </div>
        </div>
    </div>
...
@section('js')
    ...
    <script>
        ...
        function getDetailColab(data) {
            ...
            document.getElementById('created_at').innerText = '{{ __('label.company.collaboration.created_at') }}: ' +
        ...
        $(document).on('click', '.btn-delete', function (e) {
            ...
            Swal.fire({
                title: "{{ __('label.company.collaboration.revoke_confirm') }}",
                icon: "warning",
