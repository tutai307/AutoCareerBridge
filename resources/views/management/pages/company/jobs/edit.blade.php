...
@section('title', __('label.company.job.edit_job'))
...
@section('content')
    <div class="container-fluid">
        <form action="{{ route('company.updateJob', $job->id) }}" method="POST">
            @csrf
            @method('PUT')
            <!-- row -->
            <div class="row">
                        ...
                        <nav style="--bs-breadcrumb-divider: '>';" aria-label="breadcrumb">
                            <ol class="breadcrumb">
                                ...
                                <li class="breadcrumb-item active" aria-current="page">
                                    {{ __('label.company.job.edit_job') }} </li>
                            </ol>
                        </nav>
                    </div>
                </div>
                <div class="col-xl-9 col-lg-8">
                    <div class="card profile-card card-bx m-b30">
                            ...
                            <div class="row">
                                ...
                                <div class="col-sm-12 m-b30">
                                    ...
                                    <input type="text" id="slug"
                                        ...
                                        value="{{ old('slug', $job->slug) }}" placeholder="Slug" readonly>
                                    @error('slug')
                                        <span class="d-block text-danger mt-2">{{ $message }}</span>
                                    @enderror
                                </div>
                                <div class="col-sm-12 m-b30">
                                    <label class="form-label">{{ __('label.company.job.detail') }} <span
                                            class="text-danger">*</span></label>
                                    <textarea name="detail" id="detail" cols="30" rows="10" class="tinymce_editor_init">{{ old('detail', $job->detail) }}</textarea>
                                    @error('detail')
                                        <span class="d-block text-danger mt-2">{{ $message }}</span>
                                    @enderror
                                </div>
                        ...
                        <div class="card-footer">
                            ...
                            <button class="btn btn-primary" type="submit">{{ __('label.company.job.update') }}</button>
                        </div>
                    </div>
                </div>
                                ...
                                <div class="row text-start">
                                    <div class="col-sm-12 m-b30 cm-content-body form excerpt">
                                        ...
                                        <select class="multi-value-select" name="major_id"
                                            ...
                                            <option value="" @if (old('major_id', $job->major_id) == '') selected @endif>
                                                {{ __('label.company.job.select_major') }}
                                            </option>
                                            @foreach ($majors as $major)
                                                <option value="{{ $major->id }}"
                                                    @if (old('major_id', $job->major_id) == $major->id) selected @endif>{{ $major->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                        @error('major_id')
                                ...
                                <div class="row text-start">
                                    <div class="col-sm-12 m-b30">
                                        ...
                                        <input type="date" class="form-control" name="end_date"
                                            value="{{ old('end_date', $job->end_date) }}" min="{{ date('Y-m-d') }}">
                                        @error('end_date')
                                            <span class="d-block text-danger mt-2">{{ $message }}</span>
                                        @enderror
                                    </div>
                                    ...
                                    <div class="col-sm-12 m-b30">
                                        ...
                                        <select class="multi-value-select" multiple="multiple" name="skill_name[]">
                                            @foreach ($skills as $skill)
                                                <option value="{{ $skill->name }}" {{ in_array($skill->id, $job->skills->pluck('id')->toArray()) ? 'selected' : '' }}>
                                                    {{ $skill->name }}
                                                </option>
                                            @endforeach
                                        </select>
