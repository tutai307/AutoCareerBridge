            ...
            <div class="card mb-4 mb-lg-0">
                <div class="card-body p-0">
                    <ul class="list-group list-group-flush rounded-3" id="account-nav">
                        ...
                        <li class="list-group-item d-flex justify-content-between align-items-center p-3 nav-item" role="presentation">
            ...
            <div class="tab-content" id="account-tabContent">
                ...
                <div class="tab-pane fade show active" id="profile" role="tabpanel" aria-labelledby="profile-tab">
                    <div class="card mb-4">
                        ...
                        <div class="card-body">
                            <form method="POST" action="{{ route('manageAccount.updateProfile') }}" enctype="multipart/form-data">
                                @csrf
                                @method('PUT')
                                ...
                                <div class="row mb-3">
                                    ...
                                    <div class="col-sm-9">
                                        <p class="form-control-plaintext">{{ Auth::guard('student')->user()->major->name }}</p>
                                    </div>
                                </div>
                                <div class="row mb-3">
                                    <label class="col-sm-3 col-form-label">Năm nhập học</label>
                                    <div class="col-sm-9">
                                        <p class="form-control-plaintext">{{ \Carbon\Carbon::parse(Auth::guard('student')->user()->entry_year)->format('d-m-Y') }}</p>
                                    </div>
                                </div>
                                <div class="row mb-3">
                                    <label class="col-sm-3 col-form-label">Năm tốt nghiệp</label>
                                    <div class="col-sm-9">
                                        <p class="form-control-plaintext">{{ \Carbon\Carbon::parse(Auth::guard('student')->user()->graduation_year)->format('d-m-Y') }}</p>
                                    </div>
                                </div>
                                ...
                                <div class="row mb-3">
                                    <label class="col-sm-3 col-form-label">Email</label>
                                     <div class="col-sm-9">
                                        <input type="email" class="form-control" name="email" value="{{ Auth::guard('student')->user()->email }}">
                                    </div>
                                </div>
                                ...
                                <div class="row mb-3">
                                    <label class="col-sm-3 col-form-label">Số điện thoại</label>
                                     <div class="col-sm-9">
                                        <input type="tel" class="form-control" name="phone" value="{{ Auth::guard('student')->user()->phone }}">
                                    </div>
                                </div>
...
.list-group-item .nav-link {
    padding: 1rem 1.25rem; /* Giữ padding gốc của list-group-item */
    color: #495057; /* Màu chữ mặc định */
    display: block; /* Đảm bảo thẻ a chiếm toàn bộ li */
...
.list-group-item .nav-link:hover {
    background-color: #f8f9fa; /* Màu nền khi hover */
     color: #0d6efd; /* Màu chữ khi hover */
}
...
{{-- Thêm script để xử lý active class khi chuyển tab nếu cần --}}
@push('scripts')
<script>
    // Đảm bảo Bootstrap JS đã được load
    // Đoạn script này có thể không cần thiết nếu bạn dùng Bootstrap bundle JS
    // var triggerTabList = [].slice.call(document.querySelectorAll('#account-nav a'))
